...
<Note>
  **Access tier: public.** This page sits in the `Getting started` group, which carries
  `"public": true` in `docs.json`. It is readable in an incognito window with no login —
  the one exemption to the site-wide authentication default.
</Note>
